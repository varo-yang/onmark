...
`film` 建立根时间域、帧率、画幅和 cue 作用域。顶层 scene 按源码顺序播放。
...
shot 是最小的默认顺序单元。它拥有一个局部时间原点；其子内容的 `delay` 都相对这个原点解释。shot 可以成为缓存候选边界，但语言不承诺它是独立 Render Unit。
