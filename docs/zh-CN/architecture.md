...
当前里程碑包含 `onmark-core`、`onmark-media`、`onmark-render`，以及 `@onmark/runtime` 已生成的 protocol 边界与顺序 session 状态机。Gate 一在首次把输入冻结、probe、compile、bundle、render 组装成用户命令时再加入 `onmark-cli`：
