...
普通 bundle 会为已求解的 video 与 overlay fact 安装一层中性的 semantic DOM
projection；它不提供 layout、style、animation、theme 或 full-screen assumption。
显式选择的 custom presentation 才拥有 DOM shape 与 runtime adapter 安装。两条路径
共用同一套确定性时钟、readiness 和媒体原语。作者侧浏览器代码的公开规则写在
...
native rendering 只在 `BeginFrame` backend 使用单独发行的
`chrome-headless-shell`，并为每个 render target 启用 CDP BeginFrameControl。
`Screenshot` backend 使用受管理的普通 Chrome headless target，不请求
BeginFrameControl；在锁定的 macOS/Windows conformance 落地前，它仍是实验性本地能力。
...
- 内部 `@onmark/motion-gsap` 因为 GSAP 依赖预算独立于 vendor-free authoring
  facade 而保持单独 package；源码 workspace 中的作者使用候选 `onmark/motion/gsap`
  facade；根 package 的 export map 独占这条映射，bundler 只通过该表统一解析
  公开的 `onmark/*` import，不逐个选择 vendor。在可安装 release artifact 与外部
  consumer check 落地前，它不是已发布的产品合约；
- `@onmark/bundler`
...
`--presentation`，它会 bundle 中性的 semantic DOM presentation，以及可选的同名
`film.css` 与 `film.motion.ts`。motion module 只导出 declarative `motion` value，固定
生成的 entry 负责安装 runtime；`--presentation` 只用于显式选择自定义 browser code。若未传
...
`@onmark/authoring` 拥有 semantic DOM 和 vendor-neutral 的 `PresentationExtension` contract。
内部 `@onmark/motion-gsap` 具有不同的第三方依赖预算，并承载 workspace 内的
`onmark/motion/gsap` facade。只有它依赖固定版本的 GSAP，把 Rust-owned interval 投影成局部 browser seconds，
seek 时抑制 callback dispatch，并在 terminal disposal 时 kill 每条 playhead。其他引擎实现
同一 extension contract。它只允许依赖 `@onmark/authoring` 与 GSAP，消费者是 authored
motion module 或 custom presentation；bundler 与 runtime 都不选择 vendor。Three.js 在出现
同样窄且通过准入的 production adapter 前，仍只是 repository development dependency。
...
projection 保持有界；默认 authoring adapter 再据此建立一棵 nested semantic DOM。TypeScript
不得从数组顺序重建 source structure，也不得重新求时。
...
owned effect。当前 bundle manifest 把封闭 capability 纳入 content identity。CLI 从自己拥有的
presentation surface 推导能力：只有无 authored CSS、无 motion 的 semantic DOM 准入 random access；
stylesheet、authored motion 与 custom presentation 保守为 sequential；只有底层 conformance bundler 接受显式 capability。固定 Linux
...
共享一个显式帧边界。portable desktop backend 只作为实验性本地 path 复用原机制；
decoded media、WebGL、异步组件、跨环境一致性与锁定平台 lifecycle 仍需证明。
