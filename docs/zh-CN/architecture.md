...
已入库的 audio-syntax eval 用四十条真实模型输出比较了语义元素 `<music>`/`<sfx>` 与泛化的
...
capability，并把两者纳入 `bundleId`。紧凑 UTF-8 JSON identity 是
`{version,entryPoint,temporalCapability,visualCapability,files}`；file 按 portable path 排序，每个 identity entry 的字段顺序固定为
...
│   └── player/                  # framework-neutral interactive preview
├── scripts/                      # 仓库专用生成与质量检查
├── deploy/
...
- `@onmark/runtime`
  因为运行在浏览器中、并被 authoring、bundler 与 preview 消费而保持独立 package；
...
- `@onmark/player`
  是由 CLI preview 与外部 application 独立消费的 framework-neutral browser
  artifact；它唯一的 product dependency 是 runtime public facade；
- `onmark-render`
...
Gate 五 browser experiment 把 GSAP、Three.js 及 Three.js declaration 固定为 repository development dependency；它们只会进入 checked conformance presentation。`@onmark/runtime`、`@onmark/authoring` 与 Rust product crate 都不依赖这些 vendor package。这样既保持 clock vendor-free，也确保实验测量真实 vendor playhead，而不是本地仿制品。测量用 presentation 现在已通过 production `FrameEffect` lifecycle 驱动这些 playhead。vendor-free interface 由 `@onmark/runtime` 独占，并在 `Seek(frame)` 内等待完成；`@onmark/authoring` 只负责把 author-owned factory 带入标准 presentation bindings。
...
`serde`/`serde_json`；它会关闭 Lambda package 的默认 runtime feature，因此 schema
...
先写全部 versioned schema，再调用该 generator；`cargo xtask eval audio`
独立重新评分已冻结的 audio language experiment；`json-schema-to-typescript`
...
```text
@onmark/player ──→ @onmark/runtime ←── @onmark/authoring
                           ↑                  ↑
                           └── @onmark/bundler ┘
```
...
artifact 并生成 manifest；player 只消费 runtime 公开面，runtime 永不依赖 authoring、bundler 或
player。Gate 一的
...
owned effect。当前 bundle manifest 把封闭 capability 纳入 content identity；未指定的 CLI 输入保守地按
sequential 执行，只有显式 `randomAccess` 才允许 Render Graph 生成 shot-scoped unit。固定 Linux
...
transform 必须先变成显式 typed fact 并取得独立证据。Rust 不猜这些 CSS 决策。
...
`conformance/layered-media-admission.md` 中经过 review 的 Gate 七准入记录已在 commit `abb391b`
通过上述门槛：wall time 中位数为使用相同有界编码线程的 baseline 的 23.95%，incremental peak RSS
中位数为 baseline 的 79.73%。这份证据只准许继续实现显式 production capability；它不会静默切换
未声明该 capability 的 presentation，也不会取消剩余的 capability 与 conformance 工作。

production branch 在一次本地 render sequence 内保留同一条 compositor process。各 Render Unit 的
...
一张 RGBA frame。production commits `d62a5d1`、`d94081f` 与 `0f2b3c1` 实现并修正了该分支。
共享 Linux CI run
[`29886364671`](https://github.com/varo-yang/onmark/actions/runs/29886364671)
在 `0f2b3c1` 上通过 quality suite 与锁定的 real-process exit fixture，Gate 七至此关闭。
...
本关只补一条 authoring feedback loop，不创建第二个 compiler、timing solver、presentation runtime 或
final-render path。preview 消费 native execution 已接受的同一份 whole-film `BrowserPlan`、immutable
presentation bundle、materialized asset 与 `@onmark/runtime` host。Rust 继续独占所有 interval 与精确
frame identity。browser playback 只能用 monotonic clock 决定何时请求下一张整数帧；不得推导 authored
interval、累积 fractional timeline state 或改变最终 render 输出。

允许新增 `@onmark/player` package，因为外部 browser application 会独立消费它，而且它是单独的 release
artifact。它唯一的 product dependency 是 `@onmark/runtime` public surface，绝不穿透 generated 或 session
internals。它不依赖 authoring 或 bundler 内部实现，不编译 screenplay source、不扫描 presentation source，
也不拥有 asset discovery。其 framework-neutral controller 只拥有一条有界 runtime-host session、精确
seek/play/pause 状态与 terminal cleanup。`onmark-cli preview` 是它的第一个 product consumer。framework
adapter 与 component marketplace 是后续 consumer，不进入 player core 的依赖。

`onmark-cli` composition root 会增加有界的本地 preview orchestration。它继续通过现有 Rust pipeline 编译并
冻结 screenplay，构建当前 presentation artifact，只 materialize 被引用的 bytes，并在 loopback address 上
serve 一个 private root。path traversal、非 loopback binding、ambient project-file access、无界 request
body 或 connection 都必须拒绝。本关不做通用 web hosting 或远程协作。

preview audio 不得变成一份用 Web Audio 重写的 native mix。CLI 使用现有有界 `FFmpeg` path，把现有
render-owned audio plan 投影成一个临时 preview asset。player 可以 seek、播放该 asset，并把它用作 clock
projection；视觉的权威身份仍是整数帧。落后的 preview 可以跳过 display frame 追赶，但不能发明中间帧，
也不能把交互 wall-clock playback 宣称为 final-render determinism。

第八关只有在一条包含 moving video、caption、authored audio、visual resource 与已准入 frame effect 的真实
Gate 七影片同时通过以下检查后才关闭：

- 非单调精确 seek 在对应 frame identity 上得到与锁定 browser-authoritative reference capture 相同的
  canonical pixels；
- play、pause、resume、end 与重复 seek 始终保留整数 frame identity，且不增长隐式 command queue；
- preview 使用 checked presentation bundle 和 materialized asset bytes；missing、changed、oversized、
  undecodable 或 unready resource 必须通过有界 typed boundary 失败；
- audio seek 与 playback 和选中视觉帧的偏差不超过一个 output-frame duration，精确 paused seek 必须落在请求帧；
- 成功与注入失败后，dispose 都释放 runtime session、media handle、server、temporary root 与 preview audio；
- 不改变 screenplay syntax、native pixel-path admission、partition rule、cloud adapter、Studio mutation API 或
  production encoder behavior。

第八关不扩大 layered native-media profile。CSS、DOM 与 WebGL layout 仍由 presentation 拥有；不得仅为了
加速就把 `cover`、`contain`、crop、transform、picture-in-picture 或 transition 复制成 Rust layout
semantics。未来任何 native layout recipe 都必须先有显式 presentation contract，并取得独立 pixel 与
performance evidence。Studio、source editing、hot-module replacement、transition 拼写与 collaborative
authoring 仍属于不同 gate。
