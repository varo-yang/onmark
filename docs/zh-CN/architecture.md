...
跨进程锁把所选 browser 安装进 content-addressed cache，并以 atomic rename 发布。
...
product import bundling 和 no-clobber output。cross-compilation 和 binary-format
检查本身不能证明 target support。
...
schema，再调用该 generator；`cargo xtask eval audio` 独立重新评分已冻结的 audio
language experiment。相邻 release scripts 负责装配公开 npm package 与准入媒体，
