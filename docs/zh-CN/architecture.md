...
Gate 五 browser experiment 把 GSAP、Three.js 及 Three.js declaration 固定为 repository development dependency；它们只会进入 checked conformance presentation。`@onmark/runtime`、`@onmark/authoring` 与 Rust product crate 都不依赖这些 vendor package。这样既保持 clock vendor-free，也确保实验测量真实 vendor playhead，而不是本地仿制品。
