...
`onmark-render` owns the heavy Chromium dependency budget. It uses `chromiumoxide` only as a CDP transport and process launcher, with `tokio` and `futures` confined to this asynchronous execution boundary. The crate supplies the executable path, viewport, request deadlines, capture-byte ceiling, and explicit shutdown; Chromium and CDP types are translated into stable render-owned errors. Browser navigation waits separately for document load and the runtime host because the transport's navigation call does not itself establish that lifecycle barrier. Gate-one conformance launches an installed Chrome against the built runtime, crosses the typed `Load`/`Prepare`/`Seek` handshake, and verifies byte-identical repeat capture. The smoke test is opt-in until CI owns a pinned Chromium image.
