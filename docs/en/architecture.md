...
The `protocol` module uses `serde` for the stable JSON boundary. Its optional `schema` feature exposes `schemars` only to the repository-owned `onmark-xtask` build tool; product binaries do not enable it. `onmark-xtask` is a separate workspace binary because it has a build-only dependency budget and is consumed only by developers and CI. It may depend on core with the `schema` feature, `schemars`, `serde_json`, and the pinned Node generator; product crates and packages never depend on it. `cargo xtask schema` writes the versioned schemas, then invokes the checked-in Node generator. `json-schema-to-typescript` emits reviewable types, Ajv emits standalone validation code at build time, and TypeScript checks the generated package. The browser runtime does not compile schemas dynamically. Exact tool versions are pinned in the lockfiles and `mise.toml`, and CI rejects stale generated artifacts.
