...
pub(super) fn generate(repository: &Path, mode: GenerationMode) -> Result<(), Box<dyn Error>> {
    let artifacts = [
        SchemaArtifact::new::<BrowserRequest>(
            "https://onmark.dev/schemas/browser-request-v5.schema.json",
            "browser-request-v5.schema.json",
        )?,
        SchemaArtifact::new::<BrowserResponse>(
            "https://onmark.dev/schemas/browser-response-v5.schema.json",
            "browser-response-v5.schema.json",
        )?,
