...
- `trim="12s..18s"` selects source time 12s through the exclusive source time 18s;
- `trim="12s.."` selects source time 12s through the natural end;
- `trim="..18s"` selects the source beginning through exclusive source time 18s;
- `speed="2x"` plays the selected source interval at twice its natural rate.

Omit `trim` or `speed` when that treatment is not requested. The media determines
the shot duration after trimming and speed are applied.
