...
An `<om-transition>` represents an overlap between the shots immediately before
and after it. Its `duration` is local to that boundary. Omit the element for a
hard cut. Use exactly the requested shot IDs, sources, order, transition
