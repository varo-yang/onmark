...
#[test]
fn snapshot_reports_authored_errors_before_environment_preflight() {
    ...
    let report: serde_json::Value =
        serde_json::from_slice(&output.stdout).expect("the snapshot report is JSON");
    assert_eq!(report["version"], 1);
    assert_eq!(report["command"], "snapshot");
    assert_eq!(report["captured"], false);
